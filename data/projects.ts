...
export const PROJECTS: Project[] = [
  ...
  },
  {
    ...
    shortDescription: 'Central authentication platform with advanced user profiling, social media integration, and dynamic badging system',
    description: 'Profilable is a centralized account management platform serving as the authentication backbone for a suite of related services including Uploadery and Pasteview. The platform enables users to register and manage their accounts in one place, then seamlessly access all connected applications using their Profilable credentials. I contributed to key features including a comprehensive profile editing system allowing users to customize their accounts, integration of social media links and custom profile links, an intelligent badging system that recognizes user achievements and engagement levels, and a sophisticated color customization engine that dynamically applies user-selected colors to profile names, links, and background elements based on their badge status. This creates a personalized, visually distinct user experience where higher-tier badges unlock enhanced customization options. The platform works alongside other developers to provide a unified identity and authentication system across multiple interconnected applications.',
    technologies: ['React', 'Node.js', 'JavaScript', 'TypeScript', 'Express', 'MongoDB', 'REST API', 'Social Media APIs'],
    categories: [PROJECT_CATEGORIES.WEB_APP],
    company: EXPERIENCES[1].company,
    images: [
    ...
    features: [
      ...
      'Social media link integration allowing users to link their social profiles',
      'Custom profile link creation for personalized user URLs and contact information',
      'Intelligent badging system that recognizes user achievements and engagement milestones',
      'Dynamic badge assignment based on user activity and platform metrics',
      'Color customization engine allowing users to select personal color schemes',
      'Conditional color application based on badge tier and user status',
      'Profile name colorization with dynamic color styling',
      'Link colorization for enhanced visual personalization',
      'Background color customization for unique profile appearance',
      'Badge-level restrictions on customization features and options',
      'Real-time synchronization of user data across connected platforms',
      'Secure credential storage and session management',
      'Multi-application OAuth-style authentication flow',
      'User profile persistence and account history tracking',
    ],
  },
];
